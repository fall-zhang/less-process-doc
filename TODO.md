...
- 懒加载
- 重构：context，单独放置在 context 文件夹内
- 使用 jotai 对全局状态进行重构
  - 使用 jotai 对文件内容进行重构
  - 读取文件内容，先触发读取文件，然后设置新的值，最后再触发渲染
- 点击工作页面，展开和收起工作区中的内容
...
- hover 时滚动鼠标滚轮，来更新项目进度，每一步 5%
- 左侧点击之后可以编辑内容
- 可以选中 Gantt 后，使用 ctrl + 鼠标滚轮，切换从小时视图到年的视图
- 可以选择颜色
- 当前时间的着色，不同的尺寸有不同的刷新时间，天的视图，按照分钟刷新，小时的视图，按照秒刷新（先全按秒，之后优化）
- 渲染的时间，为一定的区间
- 如果左侧有事件，可以点击然后进行跳转
  - 并且可以 hover 展示详情
...
- 添加用于测试的文件，用来测试 markdown 中的解析效果：image、代码块、js、css、html、python、表格、list、todo、list、标题 h1-h6、mark 标记、strong，强调
- dialog 用于打开原生文件夹
- 行内编辑实现 `` ** 等字符的显示
- 删除图片功能有了，如何添加图片呢
